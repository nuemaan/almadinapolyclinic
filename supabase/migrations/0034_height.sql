-- ============================================================================
-- Phase 31 — intake height (for growth assessment).
-- Height is captured per-visit (like weight/temperature) so the doctor's consult
-- can compute WHO growth z-scores (weight-for-age, height-for-age, BMI-for-age).
--   * visits.height        — intake height/length in cm
--   * session_queue()       — now returns it
--   * reception_update()    — gains p_height
-- ============================================================================
...
drop function if exists reception_update(uuid, text, text, int, int, int, text, text);
...
begin
  ...
  update visits
     set weight      = nullif(btrim(coalesce(p_weight, '')), ''),
         ...
         height      = nullif(btrim(coalesce(p_height, '')), '')
   where id = p_visit_id;
  return v_patient;
end;
$$;
revoke execute on function reception_update(uuid, text, text, int, int, int, text, text, text) from public, anon;
grant  execute on function reception_update(uuid, text, text, int, int, int, text, text, text) to authenticated;
