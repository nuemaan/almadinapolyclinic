-- ============================================================================
-- Phase 12 — temperature, age unit, edit-after-save, 3-day pharmacist view +
-- new/follow-up tag. Idempotent.
-- ============================================================================

alter table patients      add column if not exists age_unit text default 'years';
alter table prescriptions add column if not exists temperature text;

-- ---- book_appointment + p_age_unit ----------------------------------------
drop function if exists book_appointment(text, text, text, text, double precision, double precision, int, text);
...
begin
  ...
  insert into patients (phone, name, name_key, age, age_unit, residence)
    values (p_phone, p_name, v_name_key, p_age, nullif(p_age_unit,''), nullif(btrim(coalesce(p_residence,'')), ''))
    on conflict (phone, name_key) do update
      set name      = excluded.name,
          age       = coalesce(excluded.age, patients.age),
          age_unit  = coalesce(excluded.age_unit, patients.age_unit),
...
$$;
grant execute on function book_appointment(text, text, text, text, double precision, double precision, int, text, text) to anon, authenticated;

-- ---- save_prescription: upsert (editable) + temperature + age_unit --------
drop function if exists save_prescription(uuid, text, jsonb, int, text, text, text, jsonb, text, text, text);
...
begin
  ...
  update patients
     set age       = coalesce(p_age, age),
         age_unit  = coalesce(nullif(p_age_unit, ''), age_unit),
...
$$;
grant execute on function save_prescription(uuid, text, jsonb, int, text, text, text, jsonb, text, text, text, text, text) to authenticated;

-- ---- prescription for one visit (to prefill the consult on edit) ----------
create or replace function prescription_for_visit(p_visit_id uuid)
returns prescriptions
language sql security definer set search_path = public stable as $$
  select * from prescriptions where visit_id = p_visit_id order by created_at desc limit 1;
$$;
grant execute on function prescription_for_visit(uuid) to authenticated;

-- ---- pharmacist list: last 3 days + new/follow-up flag --------------------
...
returns table (
  ...
  patient_name text, phone text, age int, age_unit text, gender text, residence text,
  token_number int, is_followup boolean
)
language sql security definer set search_path = public stable as $$
  select pr.id, pr.created_at, pr.doctor_name, pr.notes, pr.medicines,
         ...
         p.name, p.phone, p.age, p.age_unit, p.gender, p.residence,
         v.token_number,
         -- Follow-up = exactly the 2nd visit within 14 days (1st = New, 3rd+ = New again,
         -- matching the pad's "valid for 2 visits within 14 days" rule). The patient's own
         -- current visit is excluded via v2.id <> pr.visit_id.
         (
