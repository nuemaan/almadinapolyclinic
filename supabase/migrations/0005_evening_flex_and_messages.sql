-- ============================================================================
-- Phase 7 migration — flexible evening close + friendly routing messages.
--
--  • Each session can have a "flex_close": the latest a patient may still be
--    slotted (the doctor can run a little late). Morning stays rigid (no flex,
--    doctor leaves for the hospital); evening flexes to 21:30 by default.
--    Booking reachability uses flex_close; the normal close is what's shown.
--  • book_appointment() now returns headline + message explaining the slot
--    (e.g. "Morning is over for today, so we booked you for this evening").
-- ============================================================================

alter table day_schedule add column if not exists am_flex_close time;
alter table day_schedule add column if not exists pm_flex_close time;

-- Weekly defaults: evening can run late to 21:30; morning is rigid.
update settings set value = $json$
  {
    "mon_sat": { "am": {"open":"09:00","close":"10:00"},
                 "pm": {"open":"17:30","close":"20:30","flex_close":"21:30"} },
    "sun":     { "am": {"open":"10:00","close":"13:30"},
                 "pm": {"open":"18:30","close":"20:00","flex_close":"21:30"} }
  }
$json$::jsonb
where key = 'weekly_hours';

drop function if exists effective_schedule(date);
create or replace function effective_schedule(d date)
returns table (am_open time, am_close time, am_flex_close time,
               pm_open time, pm_close time, pm_flex_close time)
language plpgsql stable set search_path = public as $$
declare r day_schedule; w jsonb; daykey text;
begin
  select * into r from day_schedule where date = d;
  if found then
    am_open := r.am_open; am_close := r.am_close; am_flex_close := r.am_flex_close;
    pm_open := r.pm_open; pm_close := r.pm_close; pm_flex_close := r.pm_flex_close;
    return next; return;
  end if;
  ...
  am_flex_close := (w->daykey->'am'->>'flex_close')::time;
  pm_open := (w->daykey->'pm'->>'open')::time; pm_close := (w->daykey->'pm'->>'close')::time;
  pm_flex_close := (w->daykey->'pm'->>'flex_close')::time;
  return next;
end;
$$;
...
declare
  ...
  v_es record; v_t record; v_d date; v_cut time; v_close_dt timestamp;
...
begin
  ...
  if p_source = 'walkin' then v_travel := 0;
  ...
  -- find earliest reachable open session (uses flex_close as the cutoff)
  ...
  if v_tdate = v_today and v_tsess = 'am' then
    ...
    v_message  := v_reason || ', so we have reserved your spot for this evening. The doctor sees evening patients a little later, so you are all set.';
  else
    ...
    v_daylabel := case when v_tdate = v_today + 1 then 'tomorrow'
                       else to_char(v_tdate, 'Dy, DD Mon') end;
