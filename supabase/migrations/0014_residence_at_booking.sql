-- ============================================================================
-- Phase 11 — patient enters Residence (R/o) at booking.
-- book_appointment() now also takes p_residence and stores it on the patient.
-- Idempotent.
-- ============================================================================

drop function if exists book_appointment(text, text, text, text, double precision, double precision, int);
...
begin
  ...
  insert into patients (phone, name, name_key, age, residence)
    values (p_phone, p_name, v_name_key, p_age, nullif(btrim(coalesce(p_residence,'')), ''))
...
$$;
grant execute on function book_appointment(text, text, text, text, double precision, double precision, int, text) to anon, authenticated;
