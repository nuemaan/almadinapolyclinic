-- ============================================================================
-- Phase 7.3 — don't say "morning is over" for a NATURAL evening booking.
-- If the patient books during evening hours (current session is already pm),
-- they weren't pushed from morning — show a plain evening message. The
-- "morning is over / closed / unreachable" explanation only appears when a
-- morning-time booking actually had to fall back to the evening. Idempotent.
-- ============================================================================
  ...
  if p_source = 'walkin' then
    ...
    if v_tdate = v_today and v_tsess = 'am' then
      ...
      if v_cur_sess = 'pm' then
        -- natural evening booking (it is already evening) — no "morning over"
        v_message := 'You are in this evening''s queue. Please reach the clinic before the session closes.';
      else
        -- morning-time booking that had to fall back to the evening
